...
| Known-person escalation/resumption | Trigger a benign event from the newly known number that needs owner review. Record the exact ID from the owner alert, restart Dark Lord, then send `APPROVE <escalation-id>` from the provisioned owner number. Repeat with `REJECT <escalation-id>` for a second benign event. | Only an `OWNER` sender can resolve the open escalation. `APPROVE` sends the persisted proposed response to the original SMS number; `REJECT` sends the safe decline response. The decision command is persisted and audited. | Not performed — requires tester action. |

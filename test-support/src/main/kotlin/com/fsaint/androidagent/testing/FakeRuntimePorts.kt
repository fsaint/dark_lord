...
class InMemoryEventStore : EventStore { val enqueued = mutableListOf<AgentEvent>(); val completed = mutableSetOf<String>(); override suspend fun enqueue(event: AgentEvent) { enqueued += event }; override suspend fun markCompleted(eventId: String) { completed += eventId } }
...
class InMemoryReplySender : ReplySender { val sent = mutableListOf<SentReply>(); override suspend fun send(recipient: String, text: String) { sent += SentReply(recipient, text) } }
...
class FakeModelProvider(private val action: PlannedAction) : ModelProvider { override suspend fun plan(session: ScopedAgentSession, event: AgentEvent, context: AgentContext): PlannedAction = action }

...
class DarkLordApplication : Application() {
    ...
    private val runtime by lazy {
        AgentRuntime(
            ...
            audit = AuditRepository(database.auditRecordDao()),
    ...
    override fun onCreate() {
        ...
        SmsBroadcastReceiverDependencies.configure(object : SmsEventSink {
            override fun publish(event: AgentEvent) = dispatch(event, "SMS")
        })
    ...
    private fun dispatch(event: AgentEvent, channel: String) {
        applicationScope.launch {
            if (channel == "SMS" && event.type == "sms.received" && event.payload["body"].isAdministrativeCommand()) {
                ...
                val result = ownerCommands.handle(sender, event.payload.getValue("body"))
                if (result.success) {
                    result.payload?.let { replies.send(event.source, it) }
                } else {
    ...
    private fun ownerStatus(): String {
        ...
        return "SMS role: ${access.smsRoleHeld}; dialer role: ${access.dialerRoleHeld}; notification access: ${access.notificationListenerEnabled}."
    }
